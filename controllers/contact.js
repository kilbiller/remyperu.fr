...
module.exports = {
  ...
  post: function(req, res, next) {
    ...
    if(!email || !nom || !message) {
      res.render('contact', {error : "Remplissez tous les champs s'il vous plaît."});
    } else {
      ...
      transporter.sendMail(mailOptions, function(error, info) {
        if(error){
          console.log(error);
          res.render('contact', {error : "Un problème a été rencontré lors de l'envoi du message."});
        }else{
          console.log('Message sent: ' + info.response);
          res.render('contact', {sucess : "Votre message a bien été envoyé."});
        }
      });
    }
  }
